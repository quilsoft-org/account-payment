...
class AccountPaymentGroup(models.Model):
    ...
    def _compute_matched_amounts(self):
        for rec in self:
            ...
            rec.matched_amount = sign * sum(
                rec.matched_move_line_ids.with_context(payment_group_id=rec.id).mapped(
    ...
    def _compute_payment_methods(self):
        ...
        for rec in self:
            ...
            rec.payment_methods = ", ".join(
                rec.payment_ids.sudo().mapped("journal_id.name")
            )
    ...
    def action_register_payment(self):
        ...
        return {
            ...
            "context": {
                ...
                "default_payment_group_company_id": self.company_id.id,
                "default_payment_type": self.partner_type == "supplier"
                and "outbound"
                or "inbound",
    ...
    def payment_print(self):
        ...
        return self.env.ref(
            "account_payment_group.action_report_payment_group"
        ).report_action(self)
    ...
    def _compute_payment_subtype(self):
        ...
        for rec in self:
            if (
                ...
                and rec.company_id.double_validation
    ...
    def _compute_payments_amount(self):
        for rec in self:
            rec.payments_amount = sum(
                rec.payment_ids.mapped("signed_amount_company_currency")
            )
            # payments_amount = sum([
        ...
        if to_pay_move_lines:
            ...
            if len(partner) != 1:
                raise ValidationError(
                    _("You can not send to pay lines from different partners")
                )
    ...
    def message_post(self, **kwargs):
        ...
        return super(
            AccountPaymentGroup, self.with_context(mail_post_autofollow=True)
        ).message_post(**kwargs)
    ...
    def action_account_invoice_payment_group(self):
        ...
        if move_ids.filtered(lambda x: x.state != "posted") or move_ids.filtered(
            lambda x: x.invoice_payment_state not in ["not_paid", "partial"]
        ):
            raise ValidationError(
                _("You can only register payment if invoice is posted and unpaid")
            )
        return {
            "name": _("Register Payment"),
            "view_type": "form",
