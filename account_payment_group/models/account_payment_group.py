...
class AccountPaymentGroup(models.Model):
    ...
    def _get_to_pay_move_lines_domain(self):
        ...
        if self.commercial_partner_id:
            commercial_partner = self.commercial_partner_id.id
        else:
            if self.partner_id:
                if self.partner_id.is_company or not self.partner_id.parent_id:
                    commercial_partner = self.partner_id.id
                else:
                    commercial_partner = self.partner_id.parent_id.id
    ...
    def _compute_matched_move_line_ids(self):
        ...
        for rec in self:
            ...
            rec.matched_move_line_ids = (payment_lines.mapped('matched_debit_ids.debit_move_id') |
                                         payment_lines.mapped('matched_credit_ids.credit_move_id')) - payment_lines
    ...
    def default_get(self, fields):
        ...
        if to_pay_move_lines:
            ...
            # raise condition not applicable
            # if len(internal_type) != 1:
            #     raise ValidationError(_('You can not send to pay lines from different account types'))
    ...
    def post(self):
        ...
        for rec in self:
            ...
            if not create_from_website and not create_from_expense:
                ...
                rec.payment_ids.filtered(lambda x: x.state == 'draft').write({'state': 'posted'})
