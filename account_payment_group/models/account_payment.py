...
class AccountPayment(models.Model):
    ...
    def _synchronize_to_moves(self, changed_fields):
        ...
        for payment in payment_other_currency:
            super(
                AccountPayment,
                payment.with_context(force_rate_to=payment.exchange_rate),
            )._synchronize_to_moves(changed_fields)
        super(AccountPayment, payment_company_currency)._synchronize_to_moves(
            changed_fields
        )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            aml_data = (
                self._context.get("counterpart_aml_dicts")
                or self._context.get("new_aml_dicts")
                or [{}]
            )
            ...
            create_from_statement = (
                self._context.get("create_from_statement")
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        ...
        if self.payment_type == "transfer":
            ...
            if (
                not self.journal_id.payment_debit_account_id
                or not self.journal_id.payment_credit_account_id
            ):
                raise UserError(
                    _(
            ...
            default_line_name = self.env["account.move.line"]._get_default_line_name(
                _("Internal Transfer")
                if self.is_internal_transfer
                else payment_display_name["%s" % (self.payment_type)],
                self.amount,
                self.currency_id,
                self.date,
            ...
            line_vals_list = [
                ...
                # Receivable / Payable.
                {
                    ...
                    "partner_id": self.partner_id.id,
                    "account_id": self.destination_journal_id.payment_credit_account_id.id
                    if liquidity_balance < 0.0
                    else self.destination_journal_id.payment_debit_account_id.id,
                },
            ]
